...
static int64_t minimumDismissTimeInterval = 1;

static int64_t maximumDismissTimeInterval = 3;
...
+(void)setup {
    ...
    //初始化参数
    if (HUD_Font==nil) HUD_Font = [NSFont systemFontOfSize:16];
//        [NSFont boldSystemFontOfSize:16];
    HUD_Padding = NSEdgeInsetsMake(10, 20, 10, 20);
    if (__allHuds==nil) __allHuds = [NSMutableArray new];
    
    if (HUD_backgroudColor==nil) HUD_backgroudColor = [NSColor colorWithRed:0 green:0 blue:0 alpha:0.8];
//        [NSColor colorWithWhite:0 alpha:0.65];
...
+(void)_createUIWithMsg:(NSString*)msg
                  image:(NSImage*)image
                 inView:(NSView*)view
               withMask:(BOOL)withMask
    ...
    if (autoHide) {
        ...
        CGFloat displayDur = [FNHUD displayDurationForString:msg];
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(displayDur * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
+ (CGFloat)displayDurationForString:(NSString*)string {
    CGFloat minimum = MAX((CGFloat)string.length * 0.06 + 0.5, minimumDismissTimeInterval);
